...
const Request = require("../../src/models/approveRequestModel");
...
//   });
  ...
  Router.post('/', async(req, res) => {
    // try {
    //   const { to,from,productID } = req.body;
    //   const obj = {
    //     from: from,
    //     productID: productID,
    //     adminId: to
    ...
    //   const shop = await Shops.updateOne({ adminId: to, "products._id": productID }, {$push: {"products.$.approveRequests": obj }});
    //   console.log(shop, "shoooo");
  ...
  Router.get("/:id",  async(req, res) => {
    try {
      const abc = await Request.find({"users.1": req.params.id});
      console.log(abc,"abc")
      res.json(abc)
      // const { from, to, productID } = req.query;
